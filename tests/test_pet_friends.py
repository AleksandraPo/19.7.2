...
def test_post_api_pets_with_valid_data(name='Гоша', animal_type='попугай', age='1',
                                       pet_photo='images\попугай_гоша.jpg'):
...
def test_delete_api_pets_pet_id():
    ...
    if len(my_pets['pets']) == 0:
        status, _ = pf.post_api_create_pet_simple(auth_key, "Вася", "таракан", "1")
...
def test_successful_update_self_pet_info(name='Жорик', animal_type='ящерица', age='2'):
...
# API должна возвращать 400, а она возвращает 200
def test_update_self_pet_invalid_age(name='Жорик', animal_type='ящерица', age=-100):
...
def test_put_api_pet_invalid_pet_id(pet_id='hello', name='Жорик', animal_type='ящерица', age=1):
...
def test_post_api_create_pet_simple_with_valid_data(name='Гоша', animal_type='попугай', age='1'):
    """Проверяем что можно добавить питомца с корректными данными"""
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    # Добавляем питомца
